...
if __name__ == '__main__':
	...
	non_eth_expert_filename = c["data_path"]+c["env_path"]+str(c["non_eth_experts_weights"])+"/"+c["expe_path"]+c["model_ext"]
	for i in range(c["nb_experts"]):
		path = c["data_path"]+c["env_path"]+str(c["experts_weights"][i])+"/"
	...
	for i in range(c["n_queries"]): 
		...
		### MADMC stuff
		exec_elicit(RATIO_NORMALIZED, objective_returns, observed_rewards, set_constraints)
		#####
